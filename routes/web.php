...
Route::name('eparaksts.')
    ->middleware(['web'])
    ->group(function() {
        Route::get(config('eparaksts.redirect'), [EparakstsController::class, 'redirect'])->name('redirect');
        Route::prefix(config('eparaksts.route_prefix'))
            ->group(function(): void {
                Route::get( '/auth/{flow?}', [EparakstsController::class, 'identificationFlow'])->name('identification');
